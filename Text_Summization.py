...
# Allow nested asyncio event loops in Jupyter Notebook
nest_asyncio.apply()

async def extract_text_from_images(images):
    loop = asyncio.get_running_loop()
    with concurrent.futures.ThreadPoolExecutor() as pool:
        tasks = [loop.run_in_executor(pool, pytesseract.image_to_string, image, 'eng') for image in images]
...
def remove_redundant_info(summary):
    redundant_pattern = re.compile(r'\b(Exhibits?:|See exhibit)\s\d+\b|/b(for more details?:|For more information)\s\d+\b', re.IGNORECASE)
...
def refine_summary(summary, apply_conditions=True):
    ...
    relevant_info = re.findall(r'(Lessor|Lessee|Landlord|LANDLORD|Tenant|TENANT|House Owner|Issuer|Servicer|Location)\s*:\s*(.+?)(?=\n)', summary)
    ...
    summary = re.sub(r'.*http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\\(\\),]|(?:%[0-9a-fA-F][0-9a-fA-F])).*', '', summary, re.IGNORECASE)
...
async def generate_summary_for_pdf(pdf_path):
    pdf_text = await extract_text_from_pdf(pdf_path)  # Await the coroutine
    json_summary_bart_pdf = await generate_summary_bart_from_pdf(pdf_text) # Await the coroutine
    print(json_summary_bart_pdf)

async def generate_summary_bart_from_pdf(pdf_text):
    ...
    summary_part1_ids = model.generate(inputs_part1, max_length=800, min_length=60, length_penalty=2.0, num_beams=4, early_stopping=True)
    ...
    summary_part2_ids = model.generate(inputs_part2, max_length=800, min_length=80, length_penalty=2.0, num_beams=4, early_stopping=True)
    ...
    summary_part3_ids = model.generate(inputs_part3, max_length=2500, min_length=150, length_penalty=2.0, num_beams=4, early_stopping=True)
...
# Example usage
pdf_path = "Lease -15221 Burger King.pdf"

# Run the coroutine function using await
asyncio.run(generate_summary_for_pdf(pdf_path))
